...
// MARK: - Timer Duration Entity
struct TimerDurationEntity: AppEntity {
    static var typeDisplayRepresentation = TypeDisplayRepresentation(name: "Timer Duration")
    static var defaultQuery = TimerDurationQuery()
    ...
    static let fiveMinutes = TimerDurationEntity(id: "5", displayName: "5 minutes", minutes: 5)
    static let tenMinutes = TimerDurationEntity(id: "10", displayName: "10 minutes", minutes: 10)
    static let fifteenMinutes = TimerDurationEntity(id: "15", displayName: "15 minutes", minutes: 15)
    static let twentyMinutes = TimerDurationEntity(id: "20", displayName: "20 minutes", minutes: 20)
    static let thirtyMinutes = TimerDurationEntity(id: "30", displayName: "30 minutes", minutes: 30)
    static let sixtyMinutes = TimerDurationEntity(id: "60", displayName: "1 hour", minutes: 60)
}

struct TimerDurationQuery: EntityQuery {
    func entities(for identifiers: [String]) async throws -> [TimerDurationEntity] {
        let allDurations = [
            TimerDurationEntity.fiveMinutes,
            TimerDurationEntity.tenMinutes,
            TimerDurationEntity.fifteenMinutes,
            TimerDurationEntity.twentyMinutes,
            TimerDurationEntity.thirtyMinutes,
            TimerDurationEntity.sixtyMinutes
        ]
        
        return allDurations.filter { identifiers.contains($0.id) }
    }
    
    func suggestedEntities() async throws -> [TimerDurationEntity] {
        [
            TimerDurationEntity.tenMinutes,
            TimerDurationEntity.twentyMinutes,
            TimerDurationEntity.thirtyMinutes
...
struct AddTimerIntent: AppIntent {
    ...
    var duration: TimerDurationEntity
    ...
    func perform() async throws -> some IntentResult {
        ...
        let durationSeconds = TimeInterval(duration.minutes * 60)
        ...
        return .result(
            dialog: IntentDialog(stringLiteral: "Timer set for \(duration.displayName)")
        )
    }
}
    ...
    static var appShortcuts: [AppShortcut] {
        ...
        AppShortcut(
            ...
            phrases: [
                ...
                "Set a \(\.$duration) timer in \(.applicationName)"
            ],
